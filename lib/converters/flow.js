...
module.exports = function(h) {
  ...
  h('ForInStatement', function(node, state, c) {
    ...
    // o += 'end';
    // return o;
    c([node.left, node.right, node.body], state);
    var body = node.body.code.replace(/\n/g, '\n  ');
    node.code = `for ${node.left.code} in pairs(${node.right.code}) do\n  ${body}\nend`;
  });
  h(['FunctionDeclaration', 'FunctionExpression', 'ArrowFunctionExpression'], function(node, state, c) {
    if (node.body.type == 'UpdateExpression') {
      node.body.isStatement = true;
    }
    c(node.body, state);
    var id = '', args = [], body;
